...
export class Texture extends ListenerSubscriber {
	...
	static activeTexture: WebGLTexture;
	static activeUnit: number = -1;
	...
	create(
		...
		this.setFiltering(gl, this.options);
		// this.bindTexture();
		// this.load(options);
	}
	...
	setUrl(
		...
		return new Promise((resolve, reject) => {
			...
			element.onload = () => {
				try {
					this.setElement(gl, element, options);
				} catch (error) {
					console.log(`Texture '${this.key}' failed to load url '${url}'`, error, options);
				}
				resolve(this);
			};
